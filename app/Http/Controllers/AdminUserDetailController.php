...
class AdminUserDetailController extends Controller
{
    ...
    public function index()
    {
        ...
        ->join('status_hubungan', 'user_detail.status_hubungan_id', '=', 'status_hubungan.id')
        ->join('pekerjaan', 'user_detail.pekerjaan_id', '=', 'pekerjaan.id')
        ->get();
        return view('admin.superadmin.user-detail', ['listUserDetail' => $listUserDetail])->with('onSide', 'userDetail');
    }
    ...
    public function add()
    {
        ...
        return view('admin.superadmin.user-detail-add', ['listUser' => $listUser, 'listDusun' => $listDusun, 'listPekerjaan' => $listPekerjaan, 'listStatusHubungan' => $listStatusHubungan])->with('onSide', 'userDetail');
    }
    ...
    public function save(Request $request)
    {
        $request->validate([
            ...
            'p_ispa' => 'required',
            'p_muntaber' => 'required',
            'p_demam_berdarah' => 'required',
            'p_campak' => 'required',
            'p_malaria' => 'required',
            'p_flu_burung' => 'required',
            'p_covid19' => 'required',
            'p_hepatitis_b' => 'required',
            'p_leptopspirosis' => 'required',
            'p_kolera' => 'required',
            'p_gizi_buruk' => 'required',
            'p_jantung' => 'required',
            'p_tbc_paru' => 'required',
            'p_kanker' => 'required',
            'p_diabetes' => 'required',
            'p_hepatitis_e' => 'required',
            'p_difteri' => 'required',
            'p_chikungunya' => 'required',
            'p_lumpuh' => 'required',
            'p_lainnya' => 'required',
        ...
        try {
            ...
            return redirect('/back-user-detail')->with('success', 'Berhasil Menambahkan Pendataan');
    ...
    public function see(UserDetail $UserDetail)
    {
        $getUserDetail = \DB::table('user_detail')
        ->select('user_detail.*', 'users.name', 'dusun.nama_dusun', 'status_hubungan.nama_hubungan', 'pekerjaan.nama_pekerjaan')
        ->join('users', 'user_detail.user_id', '=', 'users.id')
        ->join('dusun', 'user_detail.dusun_id', '=', 'dusun.id')
        ->join('status_hubungan', 'user_detail.status_hubungan_id', '=', 'status_hubungan.id')
        ->join('pekerjaan', 'user_detail.pekerjaan_id', '=', 'pekerjaan.id')
        ->where('user_detail.id', '=', $UserDetail->id)
        ->first();

        $getUserPenyakit = RiwayatPenyakit::where('user_detail_id', '=', $UserDetail->id)->first();
        $getUserFasilitas = RiwayatFasilitasKesehatan::where('user_detail_id', '=', $UserDetail->id)->first();
        $getUserFisik = RiwayatAktivitasFisik::where('user_detail_id', '=', $UserDetail->id)->first();
        $getUserPsikis = RiwayatAktivitasPsikis::where('user_detail_id', '=', $UserDetail->id)->first();
        $getUserTidur = RiwayatKualitasTidur::where('user_detail_id', '=', $UserDetail->id)->first();

        $getPhbsAirBersih = PhbsAirBersih::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsMencuciTangan = PhbsMencuciTangan::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsJambanSehat = PhbsJambanSehat::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsJentikNyamuk = PhbsJentikNyamuk::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsBuahSayur = PhbsBuahSayur::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsAktivitasFisik = PhbsAktivitasFisik::where('user_detail_id', '=', $UserDetail->id)->first();
        $getPhbsRokok = PhbsMerokok::where('user_detail_id', '=', $UserDetail->id)->first();
